<?php

require_once 'dbLog.php';
$img = $_REQUEST['img'];
$age = $_REQUEST['age'];
$hobbies = $_REQUEST['hobbies'];
$firstname = $_REQUEST["firstname"];
$surname = $_REQUEST["surname"];
$city = $_REQUEST["city"];
$birthdate = $_REQUEST['birthdate'];
$mail = $_REQUEST['mail'];
$genre = $_REQUEST['genre'];
$passwords = $_REQUEST['password'];
$password = password_hash($passwords, PASSWORD_DEFAULT);

if($img == null){
    $img = "data:image/png;base64,";
}else{
    $img = $img;
}
if ($age == true) {
    $db = new DB;
    $sendData = $db->connection();
    $tmp = $sendData->query("INSERT INTO user (name, surname, birthdate, gender, city, mail, picture, password) VALUES ('$firstname', '$surname', '$birthdate', '$genre', '$city', '$mail', '$img', '$password');");
    $id_user = $sendData->lastInsertId();
    foreach ($hobbies as $hobbie) {
        $query = $sendData->query("SELECT id from hobbie where name like '$hobbie'");
        $row = $query->fetch(PDO::FETCH_ASSOC);
        $id_hobbie = $row['id'];
        $sendData->query("INSERT INTO user_has_hobbies (id_user, id_hobbies) VALUES ($id_user, $id_hobbie);");
    }
}
